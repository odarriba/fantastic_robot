...
class SendPhotoTest < ActiveSupport::TestCase
  ...
  def test_model_with_required_attributes
    model_with_required_attributes(@model_name, [:chat_id, :photo])
  end
  ...
  def test_method
    assert_equal(:sendPhoto, @subject.method)
  end
end

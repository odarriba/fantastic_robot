...
class SendVoiceTest < ActiveSupport::TestCase
  ...
  def test_model_with_required_attributes
    model_with_required_attributes(@model_name, [:chat_id, :voice])
  end
  ...
  def test_method
    assert_equal(:sendVoice, @subject.method)
  end
end

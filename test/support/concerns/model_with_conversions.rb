    ...
    def model_with_conversions(model_name, fields_factories = {})
      fields_factories.each do |field_name, factory_name|
        ...
        if (factory_name.is_a?(Array) && !factory_name.blank?)
          ...
          # Build the samples
          (0..samples).each do |i|
            objects_hash << FactoryGirl.attributes_for(factory_name.first)
            objects_built << FactoryGirl.build(factory_name.first)
          end
          ...
          object_hash = FactoryGirl.attributes_for(factory_name)
          object_build = FactoryGirl.build(factory_name)
